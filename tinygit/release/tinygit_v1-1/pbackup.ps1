...
$AlwaysExclude = @(".pbackup", ".*", "AI-NOACCESS.txt", "AI-ACCESSIBLE.txt", "ai_persistent_files", "dev", "skip-ai-persistence.txt", "*.code-workspace")
...
function Invoke-PBackupInit($defaultDest = $null) {
    if ($defaultDest) {
        $dest = Read-Host "destination (default: $defaultDest)"
...
elseif ($arg -eq "init") {
    ...
    Invoke-PBackupInit $defaultDest
}
else {
    Invoke-PBackup $arg
}
